...
Static Resolution output (the resolved YAML the tool states as fact) must match real GitLab semantics exactly — the correctness harness / oracle comparison is the gate, and any divergence is a bug, not a documentable approximation. Approximations are only permitted in the Simulation layer, where the tri-state (true/false/unknown) evaluator already labels uncertainty explicitly (e.g. `changes:`/`exists:` always unknown, default-branch treated as {main, master} union).

**Amendment (2026-07-21):** this ADR originally claimed the resolver's include merge (deep-merge everywhere) diverged from a supposed GitLab main-file-overrides-top-level-keys rule and had to be fixed. CI Lint probes against a real GitLab instance (`test/include-merge-probe.mjs`, `test/include-merge-probe2.mjs`) falsified that claim: GitLab deep-merges includes at the job level, top-level `variables`, and `default` — sibling keys from the include survive a main-file override. The resolver's include merge is oracle-verified GitLab-exact; no fix was needed.
...
- Any future semantic fix ships as its own commit on top of behavior-identical refactors, so snapshot-fixture diffs attribute every output change to the semantic fix alone. (The include-merge fix this rule was written for turned out unnecessary — see the amendment above — but the rule stands.)
